...
Plan.createMyUI = function () {
  ...
  $("main").addEventListener("mousemove", function (ev) {
    ...
    if (
      mouse.isDown &&
      Math.abs($("bookFace").offsetLeft) < UI.deviceWidth / 5
    ) {
  ...
  $("main").addEventListener("mouseup", function (ev) {
    ...
    if (Math.abs(offsetLeft) > mini) {
      ...
      //end if Math.abs(mouse.deltaX) > mini,else 则需要书图回归原位
      setTimeout(function () {
        $("bookFace").style.left = "0px";
      }, 200);
    }
  ...
  $("main").addEventListener("touchstart", function (e) {
    ...
    console.log(
      "touch start at:(" + touch.x0 + ", " + e.touches[0].pageY + ")"
    );
  ...
  $("handleBook").addEventListener(
    ...
    function (e) {
      ...
      if (!Model.bookIsOpen) {
        ...
        function playVideoBook() {
          ...
          UI.log(
            $("book"),
            "播放NO." + (i + 1) + " / " + videos.files.length + "号视频！"
          );
        ...
        function playAudioBook() {
          ...
          UI.log(
            $("book"),
            "进入 Part " + (i + 1) + " / " + audios.files.length + "的学习！"
          );
        ...
        function mediaPlayer(mediaDom, url) {
          ...
          mediaDom.addEventListener("loadedmetadata", function () {
            ...
            let s = Math.ceil(
              this.duration - parseInt(this.duration / 60) * 60
            );
          ...
          mediaDom.addEventListener("canplaythrough", function () {
            ...
            Model.clock = setInterval(() => {
              let leftTime = 0;
              if (mediaDom.duration && mediaDom.currentTime) {
                //加上这个条件，是适应低网速和大尺寸的媒体，避免频繁输出NaN信息间隔时间，造成duration输出造成用户困惑的问题
    ...
    true
  ); //$('handleBook').addEventListener("click" 。。。 最后这个true参数很重要，让该click事件不再传递到父元素main上
